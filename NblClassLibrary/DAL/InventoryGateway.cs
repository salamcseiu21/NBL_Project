...
namespace NblClassLibrary.DAL
{
    public class InventoryGateway:DbGateway
    {
        ...
        internal IEnumerable<TransactionModel> GetAllReceiveableProductToBranchByDeliveryRef(string deliveryRef)
        {
            try
            {
        ...
        internal int GetStockQtyByBranchAndProductId(int branchId, int productId)
        {
            try
            {
                int stockQty = 0;
        ...
        internal int Save(List<InvoiceDetails> invoicedOrders, Delivery aDelivery, int invoiceStatus,int orderStatus)
        {
            ConnectionObj.Open();
            SqlTransaction sqlTransaction = ConnectionObj.BeginTransaction();
            try
